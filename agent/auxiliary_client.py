...
# Legacy alias — static headers without response caching.
# Callers that need config-aware caching headers should use build_or_headers().
_OR_HEADERS = dict(_OR_HEADERS_BASE)


def build_or_headers(or_config: dict | None = None) -> dict:
    """Build OpenRouter headers, optionally including response-cache headers.

    *or_config* is the ``openrouter`` section from config.yaml (a dict with
    ``response_cache`` and ``response_cache_ttl`` keys).  When *None*, falls
    back to reading config from disk via ``load_config()``.
    """
    ...
    if or_config is None:
        try:
            ...
            or_config = {}
    if or_config.get("response_cache", False):
